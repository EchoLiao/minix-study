...
PUBLIC int do_exec()
{
    ...
    rmp = mp;
    stk_bytes = (vir_bytes) m_in.stack_bytes; /* m_in是在那赋值的 ????? */
...
int *ft;			/* place to return ft number *//* separate I & D or not */
...
vir_bytes *pc;			/* program entry point (initial PC) */
{
    ...
    /* Check magic number, cpu type, and flags for executable file. */
    ...
    *pc = hdr.a_entry;	/* initial address to start execution */
    ...
    /* 在非separate I&D中, totc并不包括text segment ????? */
    s_vir = dvir + (totc - sc); /* 除stack段以外的空间 */
#if (CHIP == INTEL && _WORD_SIZE == 2)
    m = size_ok(*ft, tc, dc, sc, dvir, s_vir);
#else
    m = (dvir + dc > s_vir) ? ENOMEM : OK; /* 有足够的空间吗? */
#endif
    ct = hdr.a_hdrlen & BYTE;		/* header length */ /* 最多只占8位 */
    ...
    vir_bytes base;			/* virtual address of stack base inside user */ /* ps */
{
    ...
    ap = (char **) stack;		/* points initially to 'nargs' */ /* argc不用relative ????? */
    ...
    int replace;
{
     ...
     *	nargs(argc) argv[0] ... argv[nargs-1] NULL envp[0] ... NULL
    ...
    /*        | argc     |                   | argc          |              */
    /*      0 |----------|                 0 |---------------|              */
    /*  (1). 传入的栈结构, 如图:    (2). 参数最少的情况(execve("/bin/ls",   */
    /*                                   "/bin/ls", NULL); )的栈结构, 如图: */
    /*      a0 == argv_p0                (即: a0 >= 4*PTRSIZE 为真的原因)   */
    /*                                                                      */
    /* 读取第2个指针的值保存于a0中, 即: argv_p0 */
    /* char *chp = ((char **) stack)[1]; a0 = (int) chp; */
    a0 = (int) ((char **) stack)[1]; /* a0 == argv_p0 == *argv == argv[0] */
